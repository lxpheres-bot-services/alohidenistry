		...
		rbx.getRoles(3008227).then((roles) => {
			rbx.getIdFromUsername(args[0]).then((result) => {
				rbx.getRankInGroup(3008227, result).then((currentRank) => {
					...
					if (roles.find((role) => role.Name === rank).Rank > roles.find((role) => role.Name === "Vice President").Rank || currentRank > roles.find((role) => role.Name === "Vice President").Rank) return message.reply("You cannot change the rank of a Vice President+, or rank anyone above Vice President.").catch(() => bot.safeSend(message, module.exports.help.name));
